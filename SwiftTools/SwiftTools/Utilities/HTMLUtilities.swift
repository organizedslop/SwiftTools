...
//
// let parsedHTML = convertHTML(rawHTML)
//
//
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
public func convertHTML(_ text: String) -> NSAttributedString?{
    ...
    if var attributedString = try? NSAttributedString(data: data, options: [.documentType: NSAttributedString.DocumentType.html], documentAttributes: nil) {
        return NSAttributedString(string: attributedString.string, attributes: [.font: UIFont.systemFont(ofSize: 17)])
    } else {
        return nil
    }
}
